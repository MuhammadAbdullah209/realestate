...
const Lisitng_edit = () => {
    ...
    const [formdata, setFormData] = useState({
        ...
        parking: false,
        furnished: false,
        imageUrls: [], // store current images
    });
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            // Step 1: Update listing fields (without images)
            ...
            // Step 2: Upload new images if selected
            if (files.length > 0) {
                const formDataToSend = new FormData();
                files.forEach(file => {
                    formDataToSend.append("images", file);
                });
                await fetch(`/Api/Listing-uploads/upload/${params.id}`, {
                    method: "POST",
                    body: formDataToSend,
                });
            }
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-10">
            <h1 className="text-3xl font-bold text-center mb-8">Edit a Listing</h1>
            ...
            {showListing && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-white w-full max-w-2xl p-6 rounded-xl shadow-lg relative max-h-[90vh] overflow-y-auto">
                        ...
                        <h2 className="text-2xl font-semibold mb-4">{formdata.name || 'Listing Preview'}</h2>
                        <p className="text-gray-600 mb-2"><strong>Address:</strong> {formdata.address}</p>
            ...
            <form onSubmit={handleUpdate} className="bg-white p-8 shadow-xl rounded-2xl space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        />
                        <p className="text-sm text-gray-600 mt-1">Bedrooms: {formdata.bedrooms}</p>
                    </div>
                    <div>
                        <input
                            type="number"
                        ...
                        />
                        <p className="text-sm text-gray-600 mt-1">Bathrooms: {formdata.bathrooms}</p>
                    </div>
                    <div>
                        <input
                            type="number"
                        ...
                        />
                        <p className="text-sm text-gray-600 mt-1">Regular Price: ${formdata.regularprice}</p>
                    </div>
                </div>
                {formdata.offer && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            />
                            <p className="text-sm text-gray-600 mt-1">Discounted Price: ${formdata.discountprice}</p>
                        </div>
                    </div>
                )}
                <div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        />
                        <button type="button" className="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 transition">Upload</button>
                    </div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {formdata.imageUrls?.map((url, index) => (
                        <div key={index} className="relative">
                            <img src={url} alt="listing" className="w-full h-32 object-cover rounded-md" />
                            <button
                                type="button"
                ...
                <button
                    ...
                    disabled={loading}
                    type="submit"
                    className={`w-full bg-blue-700 text-white py-3 rounded-lg transition text-lg font-semibold
